...
import com.mozhimen.opencv453.opencvk.utils.MatchUtil
import com.mozhimen.opencv453.opencvk.utils.MatFormatUtil
import com.mozhimen.opencv453.opencvk.libs.OpenCVKLib
import com.mozhimen.opencv453.opencvk.utils.setImageMat
...
class OpenCVKMatchActivity : BaseActivityVB<ActivityOpencvkMatchBinding>() {
    private val _templateMat by lazy { MatFormatUtil.bitmap2mat(UtilKRes.gainDrawable(R.mipmap.opencvk_contrast_test)!!.drawable2bitmap()) }
        ...
        object : ICameraXKFrameListener {
            ...
            override fun invoke(imageProxy: ImageProxy) {
                try {
                    ...
                    val srcMat = MatFormatUtil.bitmap2mat(cropBitmap)

                    val resultMat = MatchUtil.templateMatch(srcMat, _templateMat)
                    Thread.sleep(100)
                    try {
                        vb.opencvkMatchImg.setImageMat(resultMat)
                    } catch (e: Exception) {
                        e.printStackTrace()
                    } finally {
                        srcMat.release()
